...
const (
	nonceSize  = 128
	headerTmpl = `UsernameToken Username="%s", PasswordDigest="%s", Nonce="%s", Created="%s"`
)

// Transport implements http.Transport. It adds X-WSSE header
// to client requests.
type Transport struct {
	Username, Password, WSSE string
...
func CreatePasswordDigest(nonce, created, password string) string {
	nonceByte, _ := base64.StdEncoding.DecodeString(nonce)

	return base64.StdEncoding.EncodeToString(createPasswordDigest(string(nonceByte), created, password))
}
